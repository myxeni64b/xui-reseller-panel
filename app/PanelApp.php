...
if ($path === '/login' && $method === 'GET') {
            ...
            $this->guestOnly(true);
            return $this->renderAuth('login.php', array('errors' => array(), 'old' => array('role' => 'admin', 'username' => '')));
        }
...
if ($path === '/reseller/profile' && $method === 'POST') { $this->resellerOnly(); $this->validateCsrf(); return $this->saveResellerPassword(); }
    ...
    {
        $role = $this->sanitizeIdentifier($this->input('role', 'admin'), 16);
        if (!in_array($role, array('admin', 'reseller'), true)) { $role = 'admin'; }
        ...
        if ($username === '' || strlen($password) < 1 || strlen($password) > 4096) {
            $this->noteLoginFailure($role, $username === '' ? 'empty' : $username);
            return $this->renderAuth('login.php', array('errors' => array('login' => array('Invalid credentials or disabled account.')), 'old' => array('role' => $role, 'username' => $username)));
        }

        $limit = $this->assertLoginRateAllowed($role, $username);
        if (!$limit['ok']) {
            return $this->renderAuth('login.php', array('errors' => array('login' => array($limit['message'])), 'old' => array('role' => $role, 'username' => $username)));
        }

        $record = $this->store->findBy($role === 'admin' ? 'admins' : 'resellers', 'username', $username);
        if (!$record || (isset($record['status']) && $record['status'] !== 'active') || !password_verify($password, isset($record['password_hash']) ? $record['password_hash'] : '')) {
            $this->noteLoginFailure($role, $username);
            return $this->renderAuth('login.php', array('errors' => array('login' => array('Invalid credentials or disabled account.')), 'old' => array('role' => $role, 'username' => $username)));
        }

        $this->clearLoginFailure($role, $username);
        session_regenerate_id(true);
    ...
    {
        $record = array('username' => '', 'display_name' => '', 'prefix' => '', 'credit_gb' => '0', 'fixed_duration_days' => $this->defaultDurationDays(), 'max_expiration_days' => $this->defaultDurationDays(), 'max_ip_limit' => 0, 'status' => 'active', 'restrict' => 0, 'notes' => '', 'api_key' => '', 'regenerate_api_key' => 0, 'allowed_template_ids' => array());
        $errors = array();
        if ($mode === 'edit') {
    ...
    protected function saveReseller($id = null)
    {
        ...
        $payload = array('username' => $data['username'], 'display_name' => $data['display_name'], 'prefix' => $data['prefix'], 'credit_gb' => (float) $data['credit_gb'], 'fixed_duration_days' => (int) $data['fixed_duration_days'], 'max_expiration_days' => (int) $data['max_expiration_days'], 'max_ip_limit' => (int) $data['max_ip_limit'], 'telegram_user_id' => $data['telegram_user_id'], 'status' => $data['status'], 'restrict' => $data['restrict'] ? 1 : 0, 'notes' => $data['notes'], 'allowed_template_ids' => $data['allowed_template_ids']);
    ...
    protected function customersPage($scope)
    {
        ...
        if ($q !== '') {
            $customers = array_values(array_filter($customers, function ($item) use ($q) {
                $blob = strtolower((isset($item['display_name']) ? $item['display_name'] : '') . ' ' . (isset($item['system_name']) ? $item['system_name'] : '') . ' ' . (isset($item['subscription_key']) ? $item['subscription_key'] : ''));
        ...
        $this->renderPanel('customers_index.php', array(
            'title' => 'Customers', 'scope' => $scope, 'customers' => $customers, 'node_map' => $this->nodeMap(), 'template_map' => $this->templateMap(), 'reseller_map' => $this->resellerMap(), 'query' => $q, 'sort' => $sort, 'reseller' => $scope === 'reseller' ? $authReseller : null,
    ...
    protected function customerForm($id = null)
    {
        ...
        $record = array('display_name' => '', 'template_id' => '', 'traffic_gb' => '1', 'ip_limit' => $maxIpLimit > 0 ? '1' : '0', 'duration_days' => (string) $this->resellerDefaultCustomerDurationDays($reseller), 'duration_mode' => 'fixed', 'status' => 'active', 'phone' => '', 'access_pin' => '', 'notes' => '');
        if ($mode === 'edit') {
    ...
    protected function renderCustomerForm($mode, $record, $errors, $reseller, $templates = null)
    {
        ...
        $this->renderPanel('customer_form.php', array('title' => $mode === 'edit' ? 'Edit customer' : 'Create customer', 'mode' => $mode, 'record' => $record, 'errors' => $errors, 'templates' => $templates, 'reseller' => $reseller, 'node_map' => $this->nodeMap(), 'max_ip_limit' => $this->resellerMaxIpLimit($reseller), 'max_expiration_days' => $this->resellerMaxExpirationDays($reseller)));
    }
    ...
    protected function saveCustomer($id = null)
    {
        ...
        if ($mode === 'create') {
            if ($hasPhone xor $hasPinInput) {
                $errors['auth'][] = 'Phone and PIN must both be filled to enable /get access, or both left blank to disable it.';
            }
        } else {
            if ($hasPhone && !$hasPinInput && !$existingHasPin) {
                $errors['auth'][] = 'Set a PIN too, or clear the phone number to disable /get access.';
            }
            if (!$hasPhone && $hasPinInput) {
                $errors['auth'][] = 'Phone number is required when setting a PIN.';
            }
        }
        $maxIpLimit = $this->resellerMaxIpLimit($reseller);
        ...
        $payload = array(
            ...
            'status' => $data['status'],
            'phone' => $data['phone'],
            'access_pin_hash' => ($data['phone'] === '' ? '' : ($data['access_pin'] !== '' ? panel_password_hash($data['access_pin']) : ($existing && isset($existing['access_pin_hash']) ? $existing['access_pin_hash'] : ''))),
            'notes' => $data['notes'],
    ...
    protected function assertPublicGetRateAllowed($phone)
    {
        ...
        $identity = strtolower($ip . '|' . $this->normalizeCustomerPhone($phone));
    ...
    protected function notePublicGetFailure($phone)
    {
        $s = $this->publicGetRateSettings();
        $ip = strtolower($this->clientIp());
        $identity = strtolower($ip . '|' . $this->normalizeCustomerPhone($phone));
    ...
    protected function clearPublicGetFailure($phone)
    {
        $ip = strtolower($this->clientIp());
        $identity = strtolower($ip . '|' . $this->normalizeCustomerPhone($phone));
    ...
    protected function findCustomersByPhoneAndPin($phone, $pin)
    {
        $phone = $this->normalizeCustomerPhone($phone);
        $pin = trim((string) $pin);
        if ($phone === '' || $pin === '') { return array(); }
        $items = $this->store->filterBy('customers', function ($row) use ($phone) {
            return isset($row['phone']) && (string) $row['phone'] === (string) $phone;
        });
        $matches = array();
    ...
    {
        $phone = $this->normalizeCustomerPhone($this->input('phone', ''));
        $pin = trim((string) $this->input('pin', ''));
        $errors = array();
        $entries = array();
        if ($this->requestMethod === 'POST') {
            $limit = $this->assertPublicGetRateAllowed($phone);
            if (!$limit['ok']) {
                $errors['auth'][] = $limit['message'];
            } else {
                if (!$this->isValidCustomerPhone($phone)) {
                    $errors['phone'][] = 'Phone must contain only digits and be between 6 and 20 numbers.';
                }
                if (!$this->isValidCustomerPin($pin)) {
                    $errors['pin'][] = 'PIN must be 1 to 6 letters or numbers.';
                }
                if (!$errors) {
                    $matches = $this->findCustomersByPhoneAndPin($phone, $pin);
                    if (empty($matches)) {
                        $this->notePublicGetFailure($phone);
                        $errors['auth'][] = 'Phone or PIN is invalid.';
                    } else {
                        $this->clearPublicGetFailure($phone);
...
{
    $row = array(
        'id' => $customer['id'], 'display_name' => $customer['display_name'], 'system_name' => $customer['system_name'], 'status' => $customer['status'], 'phone' => isset($customer['phone']) ? $customer['phone'] : '',
