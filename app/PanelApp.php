...
if ($path === '/reseller/profile' && $method === 'POST') { $this->resellerOnly(); $this->validateCsrf(); return $this->saveResellerPassword(); }
    ...
    {
        $record = array('username' => '', 'display_name' => '', 'prefix' => '', 'credit_gb' => '0', 'fixed_duration_days' => $this->defaultDurationDays(), 'max_expiration_days' => $this->defaultDurationDays(), 'max_ip_limit' => 0, 'min_customer_traffic_gb' => '0', 'max_customer_traffic_gb' => '0', 'allow_fractional_traffic_gb' => 1, 'status' => 'active', 'restrict' => 0, 'notes' => '', 'api_key' => '', 'regenerate_api_key' => 0, 'allowed_template_ids' => array());
        $errors = array();
        if ($mode === 'edit') {
    ...
    protected function syncCustomersList($scope)
    {
        ...
        if ($ok > 0 && $failed === 0 && $suspectedMissing === 0) {
            if ($removed > 0 && $removed === $ok) {
                $this->flash('success', $removed . ' customer(s) were no longer present on 3x-ui and were marked Removed locally.');
            } elseif ($removed > 0) {
    ...
    protected function saveCustomer($id = null)
    {
        ...
        if ($existing) {
            $systemName = $this->replaceTrafficMarkerInClientName($existing['system_name'], $traffic);
        } else {
            $nameParts = array();
        ...
        $remoteEmail = $existing && isset($existing['remote_email']) ? $this->replaceTrafficMarkerInClientName($existing['remote_email'], $traffic) : $systemName;
    ...
    protected function markCustomerRemovedLocally($customer, $template, $node, $updateStore, $reason)
    {
        ...
        if ($reason === '') {
            $reason = 'Client no longer exists on the remote 3x-ui node.';
        }
        $message = 'Remote client was not found on 3x-ui and has been marked Removed locally.';
    ...
    protected function replaceTrafficMarkerInClientName($name, $trafficGb)
    {
        ...
        if (preg_match('/(^|-)\d+(?:\.\d+)?gb(?=-|$)/i', $name)) {
            return preg_replace('/(^|-)\d+(?:\.\d+)?gb(?=-|$)/i', '$1' . $replacement, $name, 1);
        }
        return $name;
    }
...
protected function createBackupArchive()
{
    ...
    protected function runCustomerStateCronSync($force)
    {
        ...
        if ($out['removed'] > 0) {
            $out['message'] .= ' ' . $out['removed'] . ' missing remote customer(s) were marked Removed locally.';
        }
    ...
    {
        return array('login_access','login_error','get_access','get_error','firewall_error','xui_access','xui_error','api_access','api_error','telegram_access','telegram_error','sync_access','sync_error','cron_access','cron_error');
    }
...
protected function panelSyncExportEndpoint($secret)
{
    ...
    if (empty($settings['enabled']) || $settings['mode'] !== 'master' || !$this->panelSyncSecretMatches($secret)) {
        $this->logSyncEvent('error', 'Panel sync export request rejected.', array('ip' => $this->clientIp()));
        return $this->json(array('ok' => false, 'message' => 'Forbidden.'), 403);
    }
    $this->logSyncEvent('access', 'Panel sync export payload generated.', array('ip' => $this->clientIp()));
...
protected function panelSyncRunEndpoint($secret)
{
    if (!$this->panelSyncSecretMatches($secret)) {
        $this->logSyncEvent('error', 'Panel sync run request rejected.', array('ip' => $this->clientIp()));
        return $this->json(array('ok' => false, 'message' => 'Forbidden.'), 403);
    }
