<div class="card public-card">
  ...
  <p class="muted-box">Enter your phone or email and your PIN to view all matching client subscriptions, configs, usage, and QR codes. Phone is the primary access method; email works as a secondary method when it is saved for the client.</p>
...
<div class="card public-card access-card">
  ...
  <div class="card-head"><h3><?php echo panel_e($customer['display_name']); ?></h3><span class="badge <?php echo panel_e($runtimeBadge); ?>"><?php echo panel_e($runtimeStatus); ?></span></div>
  <?php if ($runtimeState !== 'active'): ?>
    <div class="alert alert-info">
      This account is currently <?php echo panel_e($runtimeStatus); ?>.
      <?php if ($runtimeState === 'removed'): ?>It no longer exists on the remote 3x-ui server and has been kept locally for record and audit visibility.<?php elseif ($runtimeState === 'ended'): ?>Its validity period has ended.<?php elseif ($runtimeState === 'depleted'): ?>Its traffic quota is fully used.<?php elseif ($runtimeState === 'disabled'): ?>It is disabled locally.<?php endif; ?>
